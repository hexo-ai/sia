#!/usr/bin/env python3
"""Reference Harbor target agent (in-container contract).

This file is the template the meta-agent imitates when SIA runs in **Harbor mode**.
Unlike the local-task agents (which read a dataset and write a submission file), a
Harbor target agent runs *inside the benchmark task's container* and must leave the
container in a state that the task's own verifier accepts.

CLI contract (provided by sia/harbor_agent.py at runtime):

    python3 target_agent.py \
        --working_dir <dir>        # operate here; produce required outputs here
        --instruction_file <path>  # the task instruction (what to accomplish)
        --log_dir <dir>            # write agent_execution.json here (trajectory)

Runtime guarantees inside the container:
  * internet is available,
  * an LLM API key is exported in the environment (e.g. ANTHROPIC_API_KEY),
  * the solver model id is in env var SIA_TASK_MODEL (falls back to --task_model),
  * the `anthropic` SDK is installed.

The agent drives a simple bash tool-use loop: it asks the model what shell commands
to run, runs them in --working_dir, feeds results back, and stops when the model
reports the task is complete. Every step is recorded to agent_execution.json so the
SIA feedback agent can analyse the run and improve this scaffold next generation.
"""
...
MAX_TURNS = 20
COMMAND_TIMEOUT = 300  # seconds per shell command
...
def run_bash(command: str, working_dir: str) -> dict:
    """Execute a shell command in working_dir, capturing output."""
    try:
        proc = subprocess.run(
            command,
            shell=True,
    ...
    return {"stdout": clip(stdout), "stderr": clip(stderr), "exit_code": code}
...
def main() -> None:
    ...
    # Trajectory log consumed by the SIA feedback agent.
    trajectory = {
        "model": model,
    ...
    for turn in range(MAX_TURNS):
        ...
        for tu in tool_uses:
            ...
            trajectory["steps"].append(
                {"turn": turn, "type": "tool_use", "command": command, "result": result}
            )
            tool_results.append(
                {
                    "type": "tool_result",
